      ...
      <div className="lg:grid lg:grid-cols-3 lg:gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          {items.length === 0 && !showForm && (
            <Card className="text-center py-8">
              ...
              <h3 className="text-base font-semibold text-slate-700 mb-2">Sin items</h3>
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div className="min-w-0">
                    <h4 className="font-semibold text-slate-900 text-sm truncate">{item.titulo || 'Sin título'}</h4>
                ...
                <div className="flex items-center gap-2 ml-2">
                  ...
                  <button onClick={() => handleEditItem(item.id)} className="p-1.5 text-slate-400 hover:text-primary hover:bg-slate-100 rounded-lg">
                    <span className="material-symbols-outlined text-sm">edit</span>
                  </button>
                  <button onClick={() => removeItem(item.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg">
          ...
          {showForm && (
            <Card>
              <h4 className="font-semibold text-slate-900 mb-3 text-sm">{editingId ? 'Editar Item' : 'Nuevo Item'}</h4>
          ...
          {!showForm && (
            <button onClick={() => { setEditingId(null); setNewItem({ titulo: '', descripcion: '', precio: '' }); setShowForm(true); }} className="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl text-slate-500 hover:border-primary hover:text-primary transition-colors flex items-center justify-center gap-2 text-sm">
        ...
        <div className="lg:col-span-1">
          <Card className="lg:sticky lg:top-6">
            <h4 className="font-semibold text-slate-900 mb-4 text-sm">Resumen</h4>
            
            {/* IVA Toggle */}
            <div className="mb-4 p-3 bg-slate-50 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-slate-600">Aplicar IVA</span>
                <button onClick={() => setConfig({ ivaEnabled: !config.ivaEnabled })} className={`w-10 h-5 rounded-full transition-colors ${config.ivaEnabled ? 'bg-primary' : 'bg-slate-300'}`}>
              ...
              {config.ivaEnabled && (
                <div className="flex items-center gap-2">
                  <input type="number" value={config.iva} onChange={e => setConfig({ iva: parseFloat(e.target.value) || 0 })} className="w-16 px-2 py-1 text-center text-sm border border-slate-200 rounded" />
